...
{
    public partial class CreateAppliance : Form
    {
        ...
        private void InsertData (string name, string description, string powerUsage, string typicalUsage, string stocks, string applianceTypeId, string monthlyCost, string energyConsumption)
        {
            try
            {
                connection.Open();

                string Query = "insert into tblAppliances(name, description, powerUsage, typicalUsage, monthlyCost, annualCost, energyConsumption, stocks, applianceTypeId) values(@name, @description, @powerUsage, @typicalUsage, @monthlyCost, @annualCost, @energyConsumption, @stocks, @applianceTypeId);";
        ...
        private void CreateAppliance_Load(object sender, EventArgs e)
        {
            try
            {
                connection.Open();
        ...
        private void BtnCreate_Click(object sender, EventArgs e)
        {
            ...
            if (!pattern.IsMatch(stocks))
            {
                MessageBox.Show("Stocks field can only contains numbers!");
                return;
            }
            ...
            if (!pattern.IsMatch(monthlyCost))
            {
                MessageBox.Show("Monthly Cost field can only contains numbers!");
                return;
            }
            ...
            if (!pattern.IsMatch(energyConsumption))
            {
                MessageBox.Show("Energy Consumption field can only contains numbers!");
                return;
            }

            InsertData(name, description, powerUsage, typicalUsage, stocks, applianceTypeId, monthlyCost, energyConsumption);
        }
